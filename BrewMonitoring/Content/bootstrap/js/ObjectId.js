...
ObjectId.prototype.toString = function () {
    ...
    var ToHex = function(InNum)
	{
		return (InNum < 0 ? (0xFFFFFFFF + InNum + 1) : InNum).toString(16);
	}

    var timestamp = ToHex(this.timestamp);
    if (timestamp.length > 8)
    	timestamp = timestamp.substr(timestamp.length - 8, timestamp.length);

    var machine =  ToHex(this.machine);
    if (machine.length > 6)
    	machine= machine.substr(machine.length - 6, machine.length);

    var pid =  ToHex(this.pid);
    if (pid.length > 4)
    	pid = pid.substr(pid.length - 4, pid.length);

    var increment =  ToHex(this.increment);
    if (increment.length > 6)
    	increment = increment.substr(increment.length - 6, increment.length);
